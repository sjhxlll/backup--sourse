...
use function Chevereto\Legacy\get_recaptcha_invisible_html;
	...
	<script type="application/ld+json">
		{
			...
			"potentialAction": {
				"@type": "SearchAction",
				"target": "<?php echo get_base_url('search/images/?q={q}'); ?>",
...
if (Handler::cond('captcha_show') && getSetting('recaptcha_version') == 3) {
    echo get_recaptcha_invisible_html();
}
?>
<script data-cfasync="false">
	document.getElementById("chevereto-js").addEventListener("load", function() {
		...
		CHV.obj.config = {
			...
			recaptcha: {
				version: <?php echo getSetting('recaptcha_version'); ?>,
				enabled: <?php echo getSettings()['recaptcha'] ? 'true' : 'false'; ?>,
				sitekey: "<?php echo getSetting('recaptcha_public_key'); ?>",
			},
			listing: {
        ...
        if (!is_prevented_route() && !Handler::cond('404') && in_array(get_route_name(), ["image", "album", "user", "settings"]) or (Handler::hasCond('dashboard_user') && Handler::cond('dashboard_user'))) {
			...
			CHV.obj.resource = {
				...
				url: "<?php echo get_route_name() === "image"
                    ? $route["url_viewer"] ?? ''
                    : $route["url"] ?? ''; ?>",
				parent_url: "<?php echo Handler::var('image') !== null ? ((Handler::var('image')['user']['is_private'] ?? false) == 1 ? get_base_url() : Handler::var('image')['album']['url'] ?? '') : (get_route_name() == 'dashboard' ? null : $route_user['url'] ?? '') ?>"
			};
			<?php
                if (isset($route_user)) { ?>
