...
class Album
{
    ...
    protected static function handleFlood(): bool|array
    {
        if (!Login::isLoggedUser() || Login::isAdmin()) {
            return false;
        }
        $flood_limit = [
            'minute' => 20,
        ...
        if ($is_flood) {
            if (!isset(session()['flood_albums_notify'], session()['flood_albums_notify'][$flood_by])) {
                try {
                    ...
                    $message_report .= strtr('Flooding IP <a href="' . get_base_url('search/images/?q=ip:%ip') . '">%ip</a>', ['%ip' => get_client_ip()]) . '<br>';
