...
class Listing
{
    ...
    public function exec()
    {
        ...
        if ($this->type == 'images') {
            $this->where = ($this->where == '' ? 'WHERE ' : ($this->where . ' AND '))
                . 'image_is_approved = ' . (int) $this->isApproved;
        }
        $joins = [
            'images' => [
        ...
        if (isset($this->params_hidden)) {
            ...
            if (!empty($whereClauses)) {
                ...
                $this->where = ($this->where == '' ? 'WHERE ' : ($this->where . ' AND ')) . $whereClauses;
            }
        }
    ...
    public static function getTabs($args = [], $autoParams = [], $expanded = false)
    {
        ...
        if (strpos($args['basename'], get_base_url()) !== false) {
            $args['basename'] = get_base_url() == $args['basename'] ? '' : str_replace_first(get_base_url() . '/', '', $args['basename']);
        }
        $semantics = [
            'recent' => [
                'icon' => 'fas fa-history',
        ...
        foreach ($iterate as $k => $v) {
            ...
            $url = (filter_var($basename, FILTER_VALIDATE_URL) ? rtrim($basename, '/') : get_base_url($basename)) . '/?' . $http_build_query;
