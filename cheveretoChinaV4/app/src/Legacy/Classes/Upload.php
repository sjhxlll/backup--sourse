...
class Upload
{
    ...
    private ?array $source_image_exif;
    ...
    protected function validateSourceFile(): void
    {
        ...
        if (Settings::get('moderatecontent') && (Settings::get('moderatecontent_block_rating') != '' || Settings::get('moderatecontent_flag_nsfw'))) {
            ...
            if ($moderateContent->isSuccess()) {
                ...
                throw new Exception('Error processing content moderation: ' . $moderateContent->errorMessage(), 610);
            }
        }
    }
        ...
        if ($is_flood) {
            if (getSetting('flood_uploads_notify') && !session()['flood_uploads_notify'][$flood_by]) {
                try {
                    ...
                    $message = strtr('Flooding IP <a href="' . get_base_url('search/images/?q=ip:%ip') . '">%ip</a>', ['%ip' => get_client_ip()]) . '<br>';
                    if ($logged_user !== []) {
