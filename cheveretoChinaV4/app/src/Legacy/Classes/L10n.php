...
class L10n
{
    ...
    protected const CHV_BASE_LANGUAGE = 'en';
    ...
    public function __construct(
        ...
        if (self::$forced_locale === '') {
            ...
            if ($autoLanguage && !Login::isLoggedUser()) {
            ...
            if (Login::isLoggedUser() || isset(cookie()['USER_SELECTED_LANG'])) {
                $user_lang = Login::isLoggedUser()
                    ? Login::getUser()['language']
                    : cookie()['USER_SELECTED_LANG'] ?? static::CHV_BASE_LANGUAGE;
                if (array_key_exists($user_lang, self::$available_languages)
                    && !array_key_exists($user_lang, self::$disabled_languages)) {
                    $locale = $user_lang;
                }
            }
        } else {
