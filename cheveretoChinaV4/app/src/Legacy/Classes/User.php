...
use function Chevereto\Legacy\G\get_root_url;
...
class User
{
    ...
    public static function insert(array $values): int
    {
        ...
        if (!Login::isAdmin() && Settings::get('notify_user_signups')) {
            ...
            $table['%edit'] = '<a href="' . get_base_url('dashboard/user/' . $user_id) . '">edit</a>';
            $table['%user'] = '<a href="' . self::getUrl($values['username']) . '">' . $values['username'] . '</a>';
            $table['%configure'] = '<a href="' . get_base_url('dashboard/settings/users') . '">dashboard/settings/users</a>';
    ...
    public static function uploadPicture(int|array $user, string $type, array|string $source): ?array
    {
        ...
        $uploaded['fileinfo']['url'] = str_replace_first(
            get_root_url(), //search
            $assetStorage['url'], //replace
            $uploaded['fileinfo']['url'] //subject
        );
