...
{
    $subject = 'System notification: ' . $args['subject'] . ' [' . get_base_url() . ']';
    $report = $args['message'];
...
function recaptcha_check(): object
{
    ...
    $re_api = json_decode(fetch_url($endpoint));
    // Mimic old reCaptcha API return
    return (object) ['is_valid' => (bool) $re_api->success];
}

function must_use_recaptcha(int $val, ?int $max = null): bool
{
    if ($max === null) {
        $max = (int) (getSetting('recaptcha_threshold') ?? 5);
    }
...
function get_redirect_url(string $url): string
{
    ...
    return get_base_url('redirect/?to=' . urlencode(encryptString($url)) . '&auth_token=' . Handler::getAuthToken());
}
...
{
    $dirname = dirname($sub);
    $local = defined('URL_APP_PUBLIC_STATIC')
...
function get_image_fileinfo(string $file): array
{
    ...
    if (!is_url($file) && defined('CHV_ROOT_URL_STATIC')) {
...
function upload_to_content_images(array $source, string $what): void
{
    ...
    foreach ($typeArr as $k => &$v) {
        $v['name'] .= '_' . number_format(round(microtime(true) * 1000), 0, '', '') . '_' . random_string(6);
    }
...
function loaderHandler(
    ...
    try {
        new XrInstance(new Xr(
            enable: Settings::get('enable_xr'),
