...
return function (Handler $handler) {
    ...
    if (in_array('language', $allowed_to_edit) and isset($POST['language']) and $logged_user['language'] !== $POST['language'] and $logged_user['id'] == $user['id'] and array_key_exists($POST['language'], L10n::getEnabledLanguages())) {
    ...
    foreach ($routes as $route => $label) {
        ...
        $tabs[$aux] = [
            'icon' => $icons[$route],
            'label' => $label,
            'url' => get_base_url(($is_dashboard_user ? ('dashboard/user/' . $user['id']) : 'settings') . ($route == $default_route ? '' : '/' . $route)),
        ...
        switch ($doing) {
            ...
            case 'account':
                ...
                if (!$is_error && $is_email_required && !empty($POST['email']) && !timing_safe_compare($user['email'] ?? '', $POST['email'])) {
                    ...
                    $email_confirm_link = get_base_url('account/change-email-confirm/?token=' . $hashed_token['public_token_format']);
