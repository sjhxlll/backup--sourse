...
return function (Handler $handler) {
    ...
    if (in_array($doing, $keysToCheck)) {
        ...
        $captcha_needed = getSettings()['recaptcha']
            ? must_use_recaptcha($request_log['day'])
            : false;
    }
        ...
        case 'resend-activation':
            ...
            if ($POST !== [] && !$is_error) {
                ...
                if (!$is_error) {
                    ...
                    if (!$is_error) {
                        ...
                        if ($confirmation_db_query) {
                            $recovery_link = get_base_url('account/' . ($doing == 'password-forgot' ? 'password-reset' : 'activate') . '/?token=' . $hashed_token['public_token_format']);
                            global $theme_mail;
                            $theme_mail = [
                                'user' => $user,
                                'link' => $recovery_link,
                ...
                if ($is_error) {
                    ...
                    if (getSettings()['recaptcha']
                        && isset($request_log)
                        && must_use_recaptcha($request_log['day'] + 1)) {
                        $captcha_needed = true;
                    }
                    if (!$error_message) {
        ...
        case 'email-needed':
            if ($POST !== [] && !$is_error) {
                ...
                if (!$is_error) {
                    ...
                    $confirmation_db_query = Confirmation::insert($array_values);
                    $activation_link = get_base_url('account/activate/?token=' . $hashed_token['public_token_format']);
                    global $theme_mail;
                    $theme_mail = [
                        'user' => $logged_user,
