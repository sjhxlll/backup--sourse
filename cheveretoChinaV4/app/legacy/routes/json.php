...
return function (Handler $handler) {
    try {
        ...
        if (in_array($doing, ['importAdd', 'importStats', 'importProcess', 'importEdit', 'importDelete']) && Login::isAdmin() == false) {
        ...
        switch ($doing) {
            ...
            case 'list': // EX 200
                ...
                if ($doing == 'get-album-contents'
                    ...
                    foreach ($listing->outputAssoc() as $v) {
                        $contents[] = array_filter_array($v, ['title', 'id_encoded', 'url', 'url_short', 'url_viewer', 'filename', 'medium', 'thumb'], 'exclusion');
                    }
            ...
            case 'edit': // EX 3X
                ...
                switch ($type) {
                    ...
                    case 'ip_ban':
                        ...
                        try {
                            $ip_already_banned = IpBan::getSingle(['ip' => $editing['ip']]);
            ...
            case 'disconnect':
                ...
                if ($delete_connection) {
                    ...
                    if ($loginCookie === Login::getSession()['type']) {
                        $config = [
                            'callback' => get_base_url('connect/' . $disconnect) . '/',
                            'providers' => [],
                        ];
            ...
            case 'importAdd':
                ...
                $import->path = $REQUEST['path'];
                if ($REQUEST['options'] != false) {
                    $import->options = $REQUEST['options'];
                }
                $import->add();
                $import->get();
                $json_array['status_code'] = 200;
        ...
        if (isset($import->id)) {
            switch ($doing) {
                case 'importStats':
                    ...
                    break;
                case 'importProcess':
                    $import->thread = (int) $REQUEST['thread'] ?: 1;
                    $import->process();
                    $json_array['status_code'] = 200;
