...
return function (Handler $handler) {
    ...
    foreach ($routes as $route => $label) {
        ...
        $route_menu[$route] = [
            'icon' => $icons[$route],
            'label' => $label,
            'url' => get_base_url($route_prefix . ($route == $default_route ? '' : '/' . $route)),
        ...
        case 'settings':
            ...
            foreach ($settings_sections as $k => $v) {
                ...
                $settings_sections[$k] = [
                    ...
                    'key' => $k,
                    'label' => $v,
                    'url' => get_base_url($route_prefix . '/settings/' . $k),
                    'current' => $current
                ];
                if ($current) {
            ...
            if ($POST !== []) {
                ...
                if (isset($POST['akismet']) && $POST['akismet'] == 1 && $validations['akismet_api_key']['validate']) {
                    $akismet = new Akismet($POST['akismet_api_key'], get_base_url());
                ...
                if (isset($POST['recaptcha']) && $POST['recaptcha'] == 1) {
                    ...
                    if (isset($handler->request()[1]) && $handler->request()[1] == 'pages') {
                        ...
                        if ($page_values !== []) {
                            if ($handler->request()[2] == 'add') {
                                ...
                                redirect(get_base_url('dashboard/settings/pages/edit/' . $page_inserted));
                            } else {
                                if (isset($page['id'])) {
