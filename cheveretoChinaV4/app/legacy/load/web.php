...
$banned_ip = IpBan::getSingle();
if ($banned_ip) {
    if (is_url($banned_ip['message'])) {
        redirect($banned_ip['message']);
    } else {
...
$hook_before = function (Handler $handler) {
    ...
    if (Login::isLoggedUser()) {
        ...
        if (sessionVar()->hasKey('challenge_two_factor')
            && !in_array($handler->getRoutePath(), ['account/two-factor', 'recaptcha-verify', 'logout'])
    ...
    $handler::setCond('show_header', !($handler::cond('maintenance') or $handler::cond('show_consent_screen')));
    ...
    if (in_array($handler->request_array()[0], $virtualizable_routes)) {
        ...
        if ($handler->request_array()[0] !== $virtual_route) {
            $virtualized_url = str_replace(get_base_url($handler->request_array()[0]), get_base_url($virtual_route), get_current_url());
