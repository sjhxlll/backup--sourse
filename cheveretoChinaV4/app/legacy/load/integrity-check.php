...
function filesystemPermissionsCheck(): void
{
    ...
    foreach ($writing_paths as $v) {
        if (!file_exists($v)) {
            ...
            if (!is_writable($v)) {
                $errors[] = 'No write permission for PHP user ' . get_current_user() . ' in <code>' . absolute_to_relative($v) . '</code> directory. CheveretoChina needs to be able to write in this directory.';
            }
        }
    }
...
function phpCheck(SystemConfig $systemConfig): void
{
    ...
    if (version_compare(PHP_VERSION, '8.0.0', '<')) {
        $errors[] = 'This server is currently running PHP version ' . PHP_VERSION . ' and CheveretoChina needs at least PHP 8.0.0 to run.';
    }
    if (ini_get('allow_url_fopen') !== '1' && !function_exists('curl_init')) {
        $errors[] = "cURL isn't installed and allow_url_fopen is disabled. CheveretoChina needs one of these to perform HTTP requests to remote servers.";
    }
